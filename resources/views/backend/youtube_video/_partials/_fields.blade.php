...
<div class="kt-section kt-section--last" :class="{'kt-section--first': !(validation_errors.length > 0)}">
    <div class="kt-section__body">
        ...
        <div class="form-group row" :class="{'validated' : errors.has('category_video_id')}">
            <label class="col-3 col-form-label">{{ __('main.category') }}</label>
            <div class="col-9">
                <select2 v-validate="'required'" :name="'category_video_id'" v-model="fData.category_video_id" :options="category_video" :form_errors="errors"  data-vv-as="{{ trans('main.category') }}" :settings="{placeholder: '{{ __('main.category') }}'}" ></select2>
            </div>
        </div>
    </div>
</div>
